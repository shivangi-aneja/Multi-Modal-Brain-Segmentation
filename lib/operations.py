...
def conv3d_WN(x, num_filters, filter_size=[3, 3, 3], stride=[1, 1, 1], pad='SAME', init_scale=1., name="conv_WN",
...
def deconv3d_WN(x, num_filters, filter_size=[2, 2, 2], stride=[2, 2, 2], pad='SAME', init_scale=1., name="deconv_WN",
                init=False, ema=None, **kwargs):
    ...
    if pad == 'SAME':
        ...
        target_shape = [xs[0], xs[1] * stride[0] + filter_size[0] - 1, xs[2] * stride[1] + filter_size[1] - 1,
                        xs[3] * stride[2] + filter_size[2] - 1, num_filters]
    with tf.variable_scope(name):
